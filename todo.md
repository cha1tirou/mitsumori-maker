...
## ⚠️ 2026-04-20: プロジェクト凍結

ケンミツ（/construction）にピボットしたため、**見積書メーカー側の未完了タスクは全て凍結**。自動化（/next コマンド / content-agent / orchestrator）でこのファイルを参照しても、未完了項目には着手しないこと。

凍結解除の判断はユーザーが明示的に指示した時のみ。以降ケンミツ側のタスク管理は別途（CLAUDE.md のケンミツ運用ガイドを参照）。
...
- [x] guide/page.tsx に医療・製造業・不動産業・清掃業・カメラマンを追加・記事数25本に更新（2026-04-17）
- [x] 業種別ガイド記事追加（清掃業）→ guide/cleaning 実装済み（2026-04-17）
- [x] 業種別ガイド記事追加（カメラマン・写真業）→ guide/photographer 実装済み（2026-04-17）
