...
function calcSubtotal(items: DeliveryItem[]): number {
...
function DeliveryPdf({ data }: { data: DeliveryData }) {
        ...
        <View style={styles.row}>
          ...
          <View style={{ width: "40%" }}>
            ...
            <View style={{ marginTop: 8 }}>
              {data.deliveryNumber ? (
                <Text style={styles.companyInfo}>
                  納品書番号: {data.deliveryNumber}
                </Text>
              ) : null}
              <Text style={styles.companyInfo}>
                発行日: {formatDate(data.deliveryDate)}
              </Text>
              {data.deliveryDueDate ? (
                <Text style={styles.companyInfo}>
                  納品日: {formatDate(data.deliveryDueDate)}
                </Text>
              ) : null}
            </View>
          </View>
...
export async function generateDeliveryPdf(data: DeliveryData): Promise<Blob> {
  const blob = await pdf(<DeliveryPdf data={data} />).toBlob();
  return blob;
}
