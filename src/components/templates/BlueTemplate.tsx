...
export default function BlueTemplate({ data }: Props) {
  const subtotal = calcSubtotal(data.items);
  const tax = calcTax(subtotal, data.taxRate);
  const total = calcTotal(subtotal, tax);

  return (
    <div className="bg-white w-[210mm] min-h-[297mm] text-[11px] leading-relaxed text-gray-800 font-sans">
      {/* 青ヘッダー */}
      <div className="bg-blue-600 text-white px-[15mm] py-6">
        <div className="flex justify-between items-end">
          <h1 className="text-2xl font-bold tracking-widest">見 積 書</h1>
      ...
      <div className="px-[15mm] py-8">
        {/* 宛先 & 自社情報 */}
        <div className="flex justify-between mb-8">
          <div className="w-[48%]">
            <div className="border-b-2 border-blue-600 pb-1 mb-3">
            ...
            {data.subject && (
              <p className="text-xs text-gray-500">件名：{data.subject}</p>
            )}
        ...
        <table className="w-full border-collapse mb-6">
          ...
          <tbody>
            {data.items.map((item, i) => (
              <tr key={item.id} className={i % 2 === 0 ? "bg-white" : "bg-blue-50/50"}>
                <td className="py-2 px-3 text-gray-400 border-b border-gray-100">{i + 1}</td>
        ...
        <div className="flex justify-end mb-8">
          <div className="w-64">
            ...
            <div className="flex justify-between py-2 border-t-2 border-blue-600 mt-1 text-sm font-bold text-blue-600">
              <span>合計（税込）</span>
