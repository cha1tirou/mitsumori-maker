...
const inputClass =
  "w-full rounded-lg border border-gray-200 px-3 py-2 text-sm transition-colors hover:border-gray-300 focus:border-purple-500";

export default function PurchaseOrderForm({ data, onChange }: Props) {
  const { save, isSaved } = useCompanyInfo(data, onChange);

  const update = <K extends keyof PurchaseOrderData>(key: K, value: PurchaseOrderData[K]) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        <SectionTitle>発注情報</SectionTitle>
      ...
      <section className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-3">
          <SectionTitle>自社情報（発注元）</SectionTitle>
          <button
            type="button"
            onClick={save}
      ...
      <section className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        ...
        <div className="space-y-3">
          ...
          <button
            onClick={addItem}
            className="w-full rounded-xl border-2 border-dashed border-gray-200 py-3 text-sm text-gray-400 hover:border-purple-500 hover:text-purple-500 transition-colors"
          >
            + 明細を追加
          </button>
        </div>
