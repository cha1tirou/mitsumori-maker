  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <GuideJsonLd
        ...
        faqs={[
          {
            question: "見積書の消費税は税込・税抜どちらで書くべきですか？",
            answer: "どちらでも問題ありませんが、BtoB取引では税抜表示が主流です。税抜金額を明細に記載し、消費税額を別途表示した上で、税込の合計金額を記載する3段階方式が最も一般的です。BtoC取引の場合は総額表示（税込表示）が義務付けられています。",
          },
          {
            question: "見積書に「消費税別途」と書くだけで大丈夫ですか？",
            answer: "法的には問題ありませんが、取引先の利便性を考えると消費税額を具体的に計算して記載するのがベストです。「消費税別途」だけでは合計金額が不明確になり、取引先が社内稟議や予算管理を行いにくくなります。",
          },
          {
            question: "インボイス制度に対応した見積書を作る必要はありますか？",
            answer: "見積書はインボイス（適格請求書）には該当しないため、法的な義務はありません。ただし、請求書作成時の転記ミス防止や取引先への信頼感向上のため、登録番号や税率ごとの消費税額を見積書にも記載しておくことを推奨します。",
          },
          {
            question: "消費税の端数はどう処理すればよいですか？",
            answer: "切り捨て・切り上げ・四捨五入のいずれでも構いませんが、インボイス制度では「税率ごとに1回」の端数処理がルールです。明細の各行ごとに端数処理するのではなく、同じ税率の合計額に対して1回だけ行いましょう。",
          },
          {
            question: "免税事業者は消費税を請求してもよいですか？",
            answer: "はい、免税事業者でも消費税を上乗せして請求すること自体は法律上認められています。ただし、取引先はインボイスを発行できない事業者からの仕入れについて仕入税額控除を全額受けられないため、取引条件の交渉が発生する可能性があります。",
          },
        ]}
      />
