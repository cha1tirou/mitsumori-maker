  ...
  const common: Step[] = [
    {
      ...
      description: isPaid
        ? "8 工種（電気・水道・内装・土木・外構・大工・左官・鳶足場）から今回の工事に近いものをタップ。「プリセットを読み込む」で代表 5 項目が自動投入されます。複数工種を扱う場合はセクションを追加して重ねられます。"
        : "8 工種（電気・水道・内装・土木・外構・大工・左官・鳶足場）から今回の工事に近いものをタップ。「プリセットを読み込む」で代表 5 項目が自動投入されます。複数工種を 1 つの見積書にまとめる「セクション追加」は有料プラン限定です。",
      icon: Wand2,
      hint: {
        label: "8工種の例",
        items: [
    ...
    },
    {
      ...
      description: isPaid
        ...
        : "改正建設業法 2025 チェッカー（一式記載・労務費未入力・工事期間/場所未記入・瑕疵担保未記載などの検知）は有料プラン限定機能です。Free プランでは右ペインに表示されません。",
      icon: CheckCircle2,
      hint: isPaid
        ? {
            label: "チェック項目",
    ...
    },
    {
      ...
      title: isPaid
        ...
        : "PDFダウンロード（通常フォーマット）",
      description: isPaid
        ? "「PDFダウンロード」をクリックして保存。改正建設業法のルールに沿った見積書（労務費・法定福利費・経費の内訳明示）が出力されます。フッターには「Compliant with Construction Industry Act 2025 (revised)」の表記が入ります。"
        : "「PDFダウンロード」をクリックして保存。Free プランは通常フォーマット（透かしなし）で出力されます。労務費・法定福利費の独立計上などの改正法対応の内訳明示は反映されません。改正法対応版を出すには有料プラン（月¥1,980）が必要です。",
      icon: Download,
      hint: null,
    },
    {
      ...
      description: isPaid
        ...
        : "「見積書を保存」でクラウドに無制限で保存できます。マイページから再編集・複製・削除ができます。なお、取引先マスタ・単価マスタ・原価/粗利分析・工事写真の添付は有料プラン（月¥1,980）でご利用いただけます。",
      icon: Save,
      hint: null,
      paidOnly: false,
    },
  ...
  const featureBlocks = isPaid
    ...
    : [
        ...
        },
        {
          icon: Download,
          title: "PDF ダウンロード",
          body: "通常フォーマットで何枚でも出力可能・クラウド保存も無制限。",
        },
      ];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-3xl mx-auto px-4 py-10">
        ...
        <p className="text-gray-600 text-sm leading-relaxed mb-8">
          {isPaid
            ? "改正建設業法 2025 のルールに沿った見積書を、最短 3 分で。労務費・法定福利費・経費の内訳明示が自動で行われた正式版 PDF を出力できます。"
            : "ケンミツの使い方を全 8 ステップで解説。Free プランでは通常フォーマットの見積書を無制限で作成・PDF 出力・クラウド保存できます。"}
        </p>
        ...
        {isPaid ? (
          ...
          <div className="bg-amber-50 border border-amber-300 rounded-xl p-4 mb-8 text-sm">
            ...
            <ul className="list-disc pl-5 text-amber-900 space-y-0.5">
              <li>見積書の作成・編集・PDF 出力・クラウド保存はすべて<strong>無制限</strong>（通常フォーマット）</li>
              <li>改正建設業法 2025 チェッカー（右ペイン）は<strong>有料プラン限定</strong></li>
              <li>改正法対応版 PDF（労務費・法定福利費の内訳明示）は<strong>有料プラン限定</strong></li>
              <li>複数工種セクション（階層見積）・工事写真の添付は<strong>有料プラン限定</strong></li>
              <li>取引先マスタ・単価マスタ・原価/粗利分析は<strong>有料プラン限定</strong></li>
            </ul>
          </div>
        )}
