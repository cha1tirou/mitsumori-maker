...
const faqCategories: FaqCategory[] = [
  {
    ...
    items: [
      {
        q: "無料で使えますか？",
        a: "はい。メールアドレスとパスワードだけで無料登録でき、見積書は何枚でも作成・PDF ダウンロード・クラウド保存できます。クレジットカード登録は不要です。改正建設業法のルールに沿った見積書を作成したい場合のみ、有料プラン（月¥1,980）にアップグレードしてください。",
      },
      {
        q: "有料プランでできるようになることは何ですか？",
        a: "改正建設業法のルールに沿った見積書（労務費・法定福利費・経費の内訳明示）の作成、リアルタイム法令チェッカー、複数工種セクション（階層見積）、取引先マスタ・単価マスタ、原価・粗利分析、工事写真の添付など。Free プランでは見積書の作成・編集・保存・通常 PDF 出力は無制限に利用できますが、上記機能は有料プラン限定です。",
      },
      {
        q: "年払いは割引がありますか？",
      ...
      {
        q: "領収書は発行されますか？",
        a: "はい。有料プラン決済時に Stripe（決済代行）から自動で領収書 PDF がメール送付されます。マイページの「領収書の発行方法」ボタンから再取得も可能です。発行者「ケンミツ」・住所・支払日・領収書番号が記載されます。",
      },
      {
        q: "どのような支払い方法に対応していますか？",
        a: "クレジットカード決済のみ対応しています（Stripe 決済）。対応カードブランドは Visa / Mastercard / JCB / American Express / Diners Club。現時点で銀行振込・コンビニ決済・法人向け請求書払い（後払い）には対応しておりません。月額¥1,980 の小額決済を前提にしているため、今後も当面はクレジットカード決済に限定する予定です。",
      },
      {
        q: "法人契約・請求書払いはできますか？",
    ...
    items: [
      {
        ...
        a: "有料プランで、2025 年 12 月施行の改正建設業法に対応した見積書を出力できます。労務費の内訳明示・「一式」表記の警告・見積条件書の必須記載事項・瑕疵担保責任のデフォルト文言を標準搭載。リアルタイムで法令準拠状況をチェックする建設業法チェッカー機能も付いています（Free プランは通常フォーマット）。",
      },
      {
        q: "どんな工種に対応していますか？",
        a: "電気・水道設備・内装・土木・外構・大工・左官・鳶足場の8工種に特化したプリセットを用意。各工種に代表的な5項目が登録されており、ワンクリックで見積の骨格が完成します。複数工種を組み合わせた階層見積は有料プラン限定機能です（Free プランは 1 工種セクション）。",
      },
      {
        q: "法定福利費は自動計算されますか？",
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: "解約後もデータは180日間保管されます。この期間中は Free プランとして引き続き閲覧・通常版 PDF 出力が可能です。改正建設業法のルールに沿った見積書の作成・編集・複製・マスタ機能は有料プラン限定機能のため利用できなくなります。180日を過ぎるとデータは完全に削除されます。再度有料プランに契約すれば、保管期間中のデータにフルアクセスが復活します。",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        a: "ログイン時は Supabase のクラウドデータベース（暗号化保管）に保存されます。見積書作成画面（/construction/new）はメールアドレス登録が必要で、未ログインのままではご利用いただけません。解約後も 180 日間はデータが保管され、その後削除されます。",
      },
      {
